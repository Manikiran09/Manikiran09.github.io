...
const TopNav = () => {
  ...
  const linkClass = (path) =>
    `rounded-full px-4 py-2 text-sm font-semibold text-white/85 transition hover:bg-white/10 hover:text-white ${
    ...
    <header className="sticky top-0 z-50 border-b border-white/20 bg-slate-950/90 shadow-2xl shadow-slate-950/20 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 py-4 md:flex-row md:items-center md:justify-between md:px-6">
        <div className="flex flex-wrap items-center gap-3">
          <Link to="/dashboard" className="flex items-center gap-3 rounded-full bg-white/10 px-4 py-2 text-lg font-bold tracking-tight text-white">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-2xl border border-white/10 bg-white/10 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/15"
